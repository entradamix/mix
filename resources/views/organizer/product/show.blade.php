        ...
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
              <a href="{{ route('organizer.shop_management.product_type') }}" class="btn btn-secondary btn-sm float-right"> <i
                  class="fas fa-plus"></i>
                {{ __('Add Product') }}
              </a>

              <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                data-href="{{ route('organizer.shop_management.product.bulk_delete') }}">
                        ...
                        <tr>
                          ...
                          <td>
                            <form id="statusForm-{{ $product->id }}" class="d-inline-block"
                              action="{{ route('organizer.shop_management.product.status_update', ['id' => $product->id, 'language' => request()->input('language')]) }}"
                              method="post">
                          ...
                          <td>
                            ...
                            <form id="featuredForm-{{ $product->id }}" class="d-inline-block"
                              action="{{ route('organizer.shop_management.product.update_feature', ['id' => $product->id]) }}"
                              method="post">
                          ...
                          <td>
                            <a href="{{ route('organizer.shop_management.product.edit', ['id' => $product->id]) }}"
                              class="btn btn-primary mt-1 btn-sm">
                              <i class="fas fa-edit"></i>
                            </a>
                            <form class="deleteForm d-block"
                              action="{{ route('organizer.shop_management.product.destroy', ['id' => $product->id]) }}"
                              method="post">
