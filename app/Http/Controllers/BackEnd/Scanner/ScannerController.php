...
namespace App\Http\Controllers\BackEnd\Scanner;
...
class ScannerController extends Controller
{
  ...
  {
    $information['total_events'] = Event::where('organizer_id', Auth::guard('scanner')->user()->organizer_id)->get()->count();
    $information['total_event_bookings'] = Booking::where('organizer_id', Auth::guard('scanner')->user()->organizer_id)->get()->count();
    $information['transcation_count'] = Transaction::where('organizer_id', Auth::guard('scanner')->user()->organizer_id)->get()->count();
    ...
    $eventBookingTotalIncomes = DB::table('bookings')
      ...
      ->where('organizer_id', Auth::guard('scanner')->user()->organizer_id)
      ->get();
    ...
    return view('scanner.index', $information);
  }
 
  //login
  public function login()
  {
    return view('frontend.scanner.login');
  }
  
  //signup
  public function signup()
  {
    return view('frontend.scanner.signup');
  }
  ...
  public function create(Request $request)
  {
    $rules = [
      ...
      'email' => 'required|email|unique:scanners',
    ...
    $setting = DB::table('basic_settings')->where('uniqid', 12345)->select('scannner_email_verification', 'scanner_admin_approval')->first();
    ...
    if ($setting->organizer_email_verification == 1) {
      ...
      $link = '<a href=' . url("scanner/email/verify?token=" . $token) . '>Click Here</a>';
    ...
    $in['scanner_id'] = $scanner->id;
    $in['language_id'] = $language->id;
    ScannerInfo::create($in);
  ...
  public function authentication(Request $request)
  {
    ...
    if (
      Auth::guard('scanner')->attempt([
        'email' => $request->email,
        'password' => $request->password
      ])
    ) {
      $authAdmin = Auth::guard('scanner')->user();
  ...
  {
    return view('frontend.scanner.forget-password');
  }
  ...
  public function forget_mail(Request $request)
  {
    ...
    $user = Scanner::where('email', $request->email)->first();
    ...
    $link = '<a href=' . url("scanner/reset-password?token=" . $token) . '>Click Here</a>';
  ...
  {
    return view('frontend.scanner.reset-password');
  }
  ...
  public function update_password(Request $request)
  {
    ...
    $email = $reset->email;
    $organizer = Scanner::where('email',  $email)->first();
  ...
  {
    return view('scanner.change-password');
  }
  ...
  public function updated_password(Request $request)
  {
    ...
    $scanner = Auth::guard('scanner')->user();
  ...
  public function edit_profile()
  {
    ...
    return view('scanner.edit-profile', compact('languages'));
  }
  ...
  public function update_profile(Request $request)
  {
    ...
    $rules = [
      'email' => [
        'required',
        Rule::unique('scanners', 'username')->ignore(Auth::guard('scanner')->user()->organizer_id)
      ],
      'username' => [
        'required',
        'alpha_dash',
    ...
    if ($request->hasFile('cover')) {
      $rules['cover']  = 'mimes:jpg,jpeg,png';
    }
    ...
    $in = $request->all();
    $scanner = Scanner::find(Auth::guard('scanner')->user()->id);
    ...
    if ($cover) {
      ...
      $directory = public_path('assets/admin/img/scanner-cover/');
      $coverName = uniqid() . '.' . $extension;
      @mkdir($directory, 0775, true);
      $cover->move($directory, $coverName);

      @unlink(public_path('assets/admin/img/scanner-cover/') . $scanner->cover);
      $in['cover'] = $coverName;
    }
    ...
    foreach ($languages as $language) {
      ...
      if (!$scanner_info) {
        ...
        $scanner_info->organizer_id = $organizer->id;
      }
  ...
  {
    Session::put('scanner_theme_version', $request->scanner_theme_version);
    return redirect()->back();
  }
  ...
  public function transaction(Request $request)
  {
    ...
    $transcations = Transaction::where('organizer_id', Auth::guard('scanner')->user()->organizer_id)
      ->when($transcation_id, function ($query) use ($transcation_id) {
        return $query->where('transcation_id', 'like', '%' . $transcation_id . '%');
      })
      ->orderBy('id', 'desc')->paginate(10);
    return view('scanner.transaction', compact('transcations'));
  }
  ...
  public function monthly_income(Request $request)
  {
    ...
    return view('scanner.income', $information);
  }
}
