...
class SupportTicketController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $collection = SupportTicket::whereIn('user_id', [Auth::guard('scanner')->user()->organizer_id, Auth::guard('scanner')->user()->id])->whereIn('user_type', ['organizer', 'scanner'])->when($status, function ($query, $status) {
        ...
        return view('scanner.support_ticket.index', compact('collection'));
    }
    ...
    public function create()
    {
        ...
        return view('scanner.support_ticket.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $in['user_id'] = Auth::guard('scanner')->user()->id;
        $in['user_type'] = 'scanner';
    ...
    public function message($id)
    {
        ...
        if ($ticket->user_id != Auth::guard('scanner')->user()->organizer_id && $ticket->user_id != Auth::guard('scanner')->user()->id) {
            return redirect()->route('scanner.dashboard');
        }
        return view('scanner.support_ticket.messages', compact('ticket'));
    }
    ...
    public function ticketreply(Request $request, $id)
    {
        ...
        $input['type'] = 3;
        $input['user_id'] = Auth::guard('scanner')->user()->id;
        ...
        SupportTicket::where('id', $id)->update([
            ...
            'status' => 2,
            'user_id' => Auth::guard('scanner')->user()->id
        ]);
    ...
    public function delete($id)
    {
        ...
        $support_ticket = SupportTicket::where([['user_id', Auth::guard('scanner')->user()->organizer_id], ['user_type', 'organizer'], ['id', $id]])->where([['user_id', Auth::guard('scanner')->user()->id], ['user_type', 'scanner'], ['id', $id]])->first();
        if ($support_ticket) {
            //delete conversation 
