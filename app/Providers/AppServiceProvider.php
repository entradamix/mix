...
class AppServiceProvider extends ServiceProvider
{
  ...
  public function boot()
  {
    ...
    if (!app()->runningInConsole()) {
      ...
      $data = DB::table('basic_settings')->select('favicon', 'website_title', 'logo', 'timezone', 'preloader', 'event_guest_checkout_status')->first();
      ...
      // send this information to only back-end view files
      View::composer('scanner.*', function ($view) {
        $language = Language::where('is_default', 1)->first();
      ...
      // send this information to only back-end view files
      View::composer('promoter.*', function ($view) {
        $language = Language::where('is_default', 1)->first();
