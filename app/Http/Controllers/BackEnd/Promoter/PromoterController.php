...
namespace App\Http\Controllers\BackEnd\Promoter;
...
class PromoterController extends Controller
{
  ...
  {
    $information['total_events'] = Event::where('organizer_id', Auth::guard('promoter')->user()->organizer_id)->get()->count();
    $information['total_event_bookings'] = Booking::where('organizer_id', Auth::guard('promoter')->user()->organizer_id)->get()->count();
    $information['transcation_count'] = Transaction::where('organizer_id', Auth::guard('promoter')->user()->organizer_id)->get()->count();
    ...
    $eventBookingTotalIncomes = DB::table('bookings')
      ...
      ->where('organizer_id', Auth::guard('promoter')->user()->organizer_id)
      ->get();
    ...
    return view('promoter.index', $information);
  }
 
  //login
  public function login()
  {
    return view('frontend.promoter.login');
  }
  
  //signup
  public function signup()
  {
    return view('frontend.promoter.signup');
  }
  ...
  public function create(Request $request)
  {
    $rules = [
      ...
      'email' => 'required|email|unique:promoters',
    ...
    $setting = DB::table('basic_settings')->where('uniqid', 12345)->select('scannner_email_verification', 'promoter_admin_approval')->first();
    ...
    if ($setting->organizer_email_verification == 1) {
      ...
      $link = '<a href=' . url("promoter/email/verify?token=" . $token) . '>Click Here</a>';
    ...
    $in['promoter_id'] = $promoter->id;
    $in['language_id'] = $language->id;
    PromoterInfo::create($in);
  ...
  public function authentication(Request $request)
  {
    ...
    if (
      Auth::guard('promoter')->attempt([
        'email' => $request->email,
        'password' => $request->password
      ])
    ) {
      $authAdmin = Auth::guard('promoter')->user();
  ...
  {
    return view('frontend.promoter.forget-password');
  }
  ...
  public function forget_mail(Request $request)
  {
    ...
    $link = '<a href=' . url("promoter/reset-password?token=" . $token) . '>Click Here</a>';
  ...
  {
    return view('frontend.promoter.reset-password');
  }
  ...
  public function update_password(Request $request)
  {
    ...
    $email = $reset->email;
    $organizer = Promoter::where('email',  $email)->first();
  ...
  {
    return view('promoter.change-password');
  }
  ...
  public function edit_profile()
  {
    ...
    return view('promoter.edit-profile', compact('languages'));
  }
  ...
  public function update_profile(Request $request)
  {
    ...
    $rules = [
      'email' => [
        'required',
        Rule::unique('promoters', 'username')->ignore(Auth::guard('promoter')->user()->organizer_id)
      ],
      'username' => [
        'required',
        'alpha_dash',
    ...
    $in = $request->all();
    $promoter = Promoter::find(Auth::guard('promoter')->user()->id);
    ...
    if ($cover) {
      ...
      $directory = public_path('assets/admin/img/promoter-cover/');
      $coverName = uniqid() . '.' . $extension;
      @mkdir($directory, 0775, true);
      $cover->move($directory, $coverName);

      @unlink(public_path('assets/admin/img/promoter-cover/') . $promoter->cover);
      $in['cover'] = $coverName;
    }
    ...
    foreach ($languages as $language) {
      ...
      if (!$promoter_info) {
        ...
        $promoter_info->organizer_id = $organizer->id;
      }
  ...
  public function pwa()
  {
    return view('promoter.pwa.index');
  }
  ...
  {
    Session::put('promoter_theme_version', $request->promoter_theme_version);
    return redirect()->back();
  }
  ...
  public function transaction(Request $request)
  {
    ...
    $transcations = Transaction::where('organizer_id', Auth::guard('promoter')->user()->organizer_id)
      ->when($transcation_id, function ($query) use ($transcation_id) {
        return $query->where('transcation_id', 'like', '%' . $transcation_id . '%');
      })
      ->orderBy('id', 'desc')->paginate(10);
    return view('promoter.transaction', compact('transcations'));
  }
  ...
  public function monthly_income(Request $request)
  {
    ...
    return view('promoter.income', $information);
  }
}
