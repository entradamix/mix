...
use App\Http\Controllers\BackEnd\Organizer\WhatsConnectController;
...
class EventBookingController extends Controller
{
  ...
  public function updatePaymentStatus(Request $request, $id)
  {
    ...
    if ($request['payment_status'] == 'completed') {
      ...
      $this->sendWhats($booking, 'Booking approved');
      ...
      $this->sendWhats($booking, 'Booking rejected');
  ...
  public function sendWhats($booking, $type){
    $whatsapp = new WhatsConnectController();

    $device = WhatsConnect::where('organizer_id', $booking->organizer_id)->first();
    
    switch($type){
        case 'Booking approved':
            $message = "Obrigado pela compra! {$booking->fname} Agradecemos sua preferência. Aqui estão seu(s) ingresso(s)";
        break;
        case 'Booking rejected':
            $message = "Olá! {$booking->fname} o status da sua compra foi atualizado, seu pedido foi rejeitado! \n\n para mais informações acesse seu painel de pedidos.";
        break;
    }
    
    if (!is_null($booking->invoice)) {
        $url = public_path("assets/admin/file/invoices/$booking->invoice");
    }else{
        $url = '';
    }

    $whatsapp->send($booking->phone, $message, $device, $url);
  }
