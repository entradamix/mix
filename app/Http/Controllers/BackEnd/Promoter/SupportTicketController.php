...
class SupportTicketController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $collection = SupportTicket::whereIn('user_id', [Auth::guard('promoter')->user()->organizer_id, Auth::guard('promoter')->user()->id])->whereIn('user_type', ['organizer', 'promoter'])->when($status, function ($query, $status) {
        ...
        return view('promoter.support_ticket.index', compact('collection'));
    }
    ...
    public function create()
    {
        ...
        return view('promoter.support_ticket.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        $in['user_id'] = Auth::guard('promoter')->user()->id;
        $in['user_type'] = 'promoter';
    ...
    public function message($id)
    {
        ...
        if ($ticket->user_id != Auth::guard('promoter')->user()->organizer_id && $ticket->user_id != Auth::guard('promoter')->user()->id) {
            return redirect()->route('promoter.dashboard');
        }
        return view('promoter.support_ticket.messages', compact('ticket'));
    }
    ...
    public function ticketreply(Request $request, $id)
    {
        ...
        $input['type'] = 3;
        $input['user_id'] = Auth::guard('promoter')->user()->id;
        ...
        SupportTicket::where('id', $id)->update([
            ...
            'status' => 2,
            'user_id' => Auth::guard('promoter')->user()->id
        ]);
    ...
    public function delete($id)
    {
        ...
        $support_ticket = SupportTicket::where([['user_id', Auth::guard('promoter')->user()->organizer_id], ['user_type', 'organizer'], ['id', $id]])->where([['user_id', Auth::guard('promoter')->user()->id], ['user_type', 'promoter'], ['id', $id]])->first();
        if ($support_ticket) {
            //delete conversation 
