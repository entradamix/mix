...
class CategoryController extends Controller
{
  public function index(Request $request)
  {
    ...
    $categories = ProductCategory::where('language_id', $language->id)
      ->orderByDesc('id');
      
    if(Auth::guard('organizer')->user()){
        $categories->where('organizer_id', Auth::guard('organizer')->user()->id);
    }

    $information['categories'] = $categories->get();

    $information['langs'] = Language::all();

    $information['themeInfo'] = DB::table('basic_settings')->select('theme_version')->first();
    if(Auth::guard('organizer')->user()){
        return view('organizer.product.category.index', $information);
    }else{
  ...
  public function store(CategoryRequest $request)
  {
    ...
    if(Auth::guard('organizer')->user()){
        $ins['organizer_id'] = Auth::guard('organizer')->user()->id;
    }
