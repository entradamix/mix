...
@section('content')
  <div class="page-header">
    ...
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="{{ route('scanner.support_tickets') }}">{{ __('All Tickets') }}</a>
      </li>
      <li class="separator">
        <i class="flaticon-right-arrow"></i>
      </li>
              ...
              @if (count($ticket->messages) > 0)
                @foreach ($ticket->messages as $reply)
                  ...
                  @else
                    @if ($reply->type == 3)
                      @php
                        $scanner = App\Models\Organizer::where('id', $reply->user_id)->first();
                      @endphp
                      <div class="item-list">
                        <div class="avatar">
                          @if ($scanner->photo)
                            <img class="avatar-img rounded-circle"
                              src="{{ asset('assets/admin/img/scanner-photo/' . $scanner->photo) }}"
                              alt="user-photo">
                          @else
                        ...
                        <div class="info-user ml-3">
                          <div class="username">{{ $scanner->username }} </div>
      ...
      <div class="col-lg-6 message-type">
        <div class="card card-round">
          <div class="card-body">
            ...
            <form action="{{ route('scanner.support_ticket.reply', $ticket->id) }}" id="ajaxform" method="POST"
                ...
                <div class="input-group">
                  <div class="custom-file">
                    <input type="file" name="file" class="custom-file-input"
                      data-href="{{ route('scanner.support_ticket.zip_file.upload') }}" name="file" id="zip_file"
                      accept=".zip">
