    ...
    <div class="container">
      <div class="row justify-content-center">
        <div class="col-lg-8">
          ...
          @endif
          <form id="login-form" name="login_form" class="login-form" action="{{ route('scanner.authentication') }}"
            method="POST">
            @csrf
            ...
            <div class="form-group mt-3 d-flex justify-content-between mb-0">
              <p>{{ __('Don`t have an account') }} ? <a class="text-info"
                  href="{{ route('scanner.signup') }}">{{ __('Signup Now') }}</a></p>
              <p><a href="{{ route('scanner.forget.password') }}">{{ __('Lost your password') }} ?</a></p>
            </div>
          </form>
        </div>
      </div>
