...
class EventController extends Controller
{
  ...
  {
    return redirect()->route('organizer.add.event.event', ['type' => 'venue']);
  }
  //online_event
  ...
  public function store(StoreRequest $request)
  {
    $event = DB::transaction(function () use ($request) {
      ...
      $in['event_type'] = $request->event_type;
      ...
      $ticketContent = $request->ticket;
      $ticketContent['event_id'] = $event->id;
      $ticketContent['event_type'] = $request->event_type;
      
      $this->ticketStore($request->replace($ticketContent));
      
      return $in;
    });

    Session::flash('success', 'Added Successfully');
    //route('organizer.event.add.ticket', ['event_id' => $event['event_id'], 'language'=> $event['language'], 'event_type' => $event['event_type']])
    return response()->json(['status' => 'success', 'redirect' => route('organizer.event.add.ticket', ['event_id' => $event['event_id'], 'language'=> $event['language'], 'event_type' => $event['event_type']])], 200);
  }
  /**
