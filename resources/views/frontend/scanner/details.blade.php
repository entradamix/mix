...
@section('pageHeading')
  {{ $admin == true ? $scanner->username : $scanner->username }}
@endsection
@section('meta-keywords', "{{ $scanner->username }}")
@section('meta-description', "$scanner->details")

@section('hero-section')
  <!-- Page Banner Start -->
  <section class="page-banner overlay pt-120 pb-125 rpt-90 rpb-95 lazy" data-bg="{{ $scanner->cover != null ? asset('assets/admin/img/scanner-cover/' . $scanner->cover) : asset('assets/admin/img/' . $basicInfo->breadcrumb) }}">
    <div class="container">
      <div class="row align-items-center">
            ...
            <div class="author mb-3">
              <figure class="author-img mb-0">
                <a href="javaScript:void(0)">
                  @if ($admin == true)
                    <img class="rounded-lg lazy" data-src="{{ asset('assets/admin/img/admins/' . $scanner->image) }}"
                      alt="Author">
                  @else
                    ...
                    @else
                      <img class="rounded-lg lazy"
                        data-src="{{ asset('assets/admin/img/scanner-photo/' . $scanner->photo) }}" alt="image">
                    @endif
                  @endif
                </a>
              </figure>
              <div class="author-info">
                <h3 class="mb-1 text-white">{{ @$scanner_info->name }}</h3>
                <h6 class="mb-1 text-white">{{ $scanner->username }}</h6>
                <span>{{ __('Member since') }} {{ date('M Y', strtotime($scanner->created_at)) }}</span>
              </div>
            </div>
        ...
        <div class="col-lg-4 text-white">
          <div class="social-style-one">
            ...
            <a target="_blank" href="{{ $scanner->facebook }}"><i class="fab fa-facebook-f"></i></a>
            <a target="_blank" href="{{ $scanner->linkedin }}"><i class="fab fa-linkedin-in"></i></a>
            <a target="_blank" href="{{ $scanner->twitter }}"><i class="fab fa-twitter"></i></a>
          </div>
        </div>
      </div>
    </div>
                        ...
                        <div class="event-item">
                          ...
                          <div class="event-content">
                            ...
                            @if ($event->organizer_id != null)
                              @php
                                $scanner = App\Models\Organizer::where('id', $event->organizer_id)->first();
                              @endphp
                              @if ($scanner)
                                <a href="{{ route('frontend.scanner.details', [$scanner->id, str_replace(' ', '-', $scanner->username)]) }}"
                                  class="organizer">{{ __('By') }}&nbsp;&nbsp;{{ $scanner->username }}</a>
                              @endif
                            @else
                              @php
                                $admin = App\Models\Admin::first();
                              @endphp
                              <a href="{{ route('frontend.scanner.details', [$admin->id, str_replace(' ', '-', $admin->username), 'admin' => 'true']) }}"
                ...
                <div class="row">
                  @php
                    ...
                    if (request()->filled('admin') && request()->input('admin') == 'true') {
                        $c_events = adminCategoryWiseEvents($category->id, $language_id, $scanner->id);
                    } else {
                        $c_events = categoryWiseEvents($category->id, $language_id, $scanner->id);
                    }
                  @endphp
                  @if (count($c_events) > 0)
              ...
              <div class="author mb-20">
                <figure class="author-img">
                  ...
                  @else
                    ...
                    @else
                      <img class="rounded-lg lazy"
                        data-src="{{ asset('assets/admin/img/scanner-photo/' . $scanner->photo) }}"
                        alt="image">
                    @endif
                  @endif
                </figure>
                <div class="author-info">
                  <h6 class="mb-1">{{ @$scanner_info->name }}</h6>
                  <span class="icon-start">{{ $scanner->username }}</span>
                </div>
              </div>
              @if ($admin == true && $scanner_info)
                @if ($scanner_info->details != null)
                  <div class="font-sm">
                    <div class="click-show">
                      <div class="show-content">
                        <b>{{ __('About') }} : </b>{{ $scanner_info->details }}
                      </div>
              ...
              <ul class="toggle-list list-unstyled mt-15 font-sm">
                <li>
                  ...
                  <span class="last font-sm">
                    @if ($admin == true)
                      {{ OrganizerEventCount($scanner->id, true) }}
                    @else
                      {{ OrganizerEventCount($scanner->id) }}
                    @endif
                  </span>
                </li>
                @if ($scanner->email != null)
                  <li>
                    <span class="first">{{ __('Email') }}</span>
                    <span class="last font-sm"><a href="mailto:{{ $scanner->email }}"
                        title="{{ $scanner->email }}">{{ $scanner->email }}</a></span>
                  </li>
                @endif

                @if ($scanner->phone != null)
                  <li>
                    <span class="first">{{ __('Phone') }}</span>
                    <span class="last font-sm"><a href="tel:{{ $scanner->phone }}"
                        title="{{ $scanner->phone }}">{{ $scanner->phone }}</a></span>
                  </li>
                @endif
                @if (@$scanner_info->city != null)
                  <li>
                    <span class="first">{{ __('City') }}</span>
                    <span class="last font-sm"><a href="tel:{{ @$scanner_info->city }}"
                        title="{{ @$scanner_info->city }}">{{ @$scanner_info->city }}</a></span>
                  </li>
                @endif

                @if (@$scanner_info->state != null)
                  <li>
                    <span class="first">{{ __('State') }}</span>
                    <span class="last font-sm"><a href="tel:{{ @$scanner_info->state }}"
                        title="{{ @$scanner_info->state }}">{{ @$scanner_info->state }}</a></span>
                  </li>
                @endif
                @if (@$scanner_info->country != null)
                  <li>
                    <span class="first">{{ __('Country') }}</span>
                    <span class="last font-sm"><a href="tel:{{ @$scanner_info->country }}"
                        title="{{ @$scanner_info->country }}">{{ @$scanner_info->country }}</a></span>
                  </li>
                @endif

                @if (@$scanner_info->address != null)
                  <li>
                    <span class="first">{{ __('Address') }}</span>
                    <span class="last font-sm">{{ @$scanner_info->address }}</span>
                  </li>
                @endif

                @if ($admin == true && $scanner->address != null)
                  <li>
                    <span class="first">{{ __('Address') }}</span>
                    <span class="last font-sm">{{ $scanner->address }}</span>
                  </li>
                @endif
        ...
        <div class="modal-body">
          <div class="contact-wrapper">
            <div class="contact-form m-0">
              <form action="{{ route('scanner.contact.send_mail') }}" method="POST" id="vendorContactForm">
                @csrf
